...
from excecoes.excecao import (
    ...
    ValorAcessoInvalidoException, MultiplosArgumentosException
)
...
class TestFracaoQuinze(TestCase, TestEstacionamentoMixin):
    ...
    @parameterized.expand(
        [("08:30", "08:56", 60), ("18:40", "19:20", 90), ("10:15", "10:30", 30)]
    )
...
class TestHoraCheia(TestCase, TestEstacionamentoMixin):
    ...
    @parameterized.expand(
        [("08:30", "9:30", 102), ("08:30", "10:30", 204), ("10:15", "12:30", 234)]
    )
    @pytest.mark.TesteFuncional
...
class TestDiariaDiurna(TestCase, TestEstacionamentoMixin):
    ...
    @parameterized.expand([("08:30", "18:30"), ("06:00", "15:01"), ("09:15", "19:30")])
    @pytest.mark.TesteFuncional
...
class TestDiariaNoturna(TestCase, TestEstacionamentoMixin):
    ...
    @parameterized.expand([("20:00", "07:00"), ("19:01", "7:59"), ("23:15", "23:30")])
    @pytest.mark.TesteFuncional
...
class TestTipoAcesso(TestCase, TestEstacionamentoMixin):
    ...
    @parameterized.expand([("Mensalista", 600), ("Evento", 50)])
    @pytest.mark.TesteFuncional
...
class TestValidacoesEstacionamento(TestCase, TestEstacionamentoMixin):
    ...
    @parameterized.expand(
        [
            ...
            (
                2,
                [("hora", "14:00", "14:15"), ("hora", "14:15", "14:30")],
                564,
            ),  # 15 min / 15 min = 60
            (
                2,
                ...
                564,
                {"placa": "AS251"},
            ),  # 15 min / 15 min = 60 (mesmo carro)
        ]
    )
    @pytest.mark.TesteExcecao
    def test_valido_estacionamento_nao_lotado(
        self, max_vagas, entradas, expected, args={}
    ):
    ...
    @parameterized.expand(
        [
            ...
            (
                2,
                [("hora", "14:00", "14:15"), ("hora", "14:10", "14:30")],
                {"placa": "AS251"},
            ),  # (mesmo carro)
        ]
    )
    ...
    @pytest.mark.TesteExcecao
    def test_descricao_em_branco_dados_acesso(
        self, placa, hora_inicial, hora_final, tipo_acesso, exc
    ):
        if hora_inicial:
        ...
        with self.assertRaises(DescricaoEmBrancoException) as e:
            self.estacionamento.calcula_preco(
                placa, hora_inicial, hora_final, tipo_acesso
            )

        self.assertIn(
            f"O(s) seguinte(s) argumento(s) está(ão) em branco: [{exc}]",
            e.exception.args[0],
        )
    ...
    def test_multiplos_dados_acesso(self):
        with self.assertRaises(MultiplosArgumentosException) as e:
            self.estacionamento.calcula_preco(
                "MFQ3A",
                self.factory._str2time("10:30"),
                self.factory._str2time("11:30"),
                "Mensalista",
            )
    ...
    @parameterized.expand(
        [
            ...
            (
                ...
                600,
                None,
                "'porcentagem_contratante', 'diaria_diurna', 'valor_evento'",
            ),
        ]
    )
    @pytest.mark.TesteExcecao
    ...
    def test_valor_acesso_invalido_estacionamento(
        ...
        self.assertIn(
            f"Não pode haver valores de acesso inválidos, existem alguns negativos: [{exc}]",
            e.exception.args[0],
        )
